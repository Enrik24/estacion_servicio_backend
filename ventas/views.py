...
from usuarios.models import LimiteConsumo


def _filtros_periodo(fecha, tipo):
    if tipo == 'DIARIO':
        return {'fecha_hora__date': fecha}
    if tipo == 'MENSUAL':
        return {'fecha_hora__year': fecha.year, 'fecha_hora__month': fecha.month}
    return None


def _mensaje_rechazo(limite, acumulado, intento, razon):
    tipo = (limite.tipo or '').lower()
    unidad = limite.unidad
    return (
        f"Compra rechazada: límite {tipo} de {unidad} {razon} "
        f"(límite: {float(limite.valor):.2f}, acumulado: {float(acumulado):.2f}, intento: {float(intento):.2f})."
    )


def _validar_compra_con_limites(*, cliente, fecha, intento_monto, intento_litros, es_lleno):
    limites = LimiteConsumo.objects.filter(
        cliente_id=cliente.id,
        is_active=True,
        tipo__in=['DIARIO', 'MENSUAL'],
    ).filter(
        Q(fecha_inicio__isnull=True) | Q(fecha_inicio__lte=fecha),
        Q(fecha_fin__isnull=True) | Q(fecha_fin__gte=fecha),
    )

    if es_lleno and limites.exists():
        return 'Compra rechazada: el cliente tiene límites activos y el despacho "Lleno" no permite validar consumo.'

    for limite in limites:
        filtros = _filtros_periodo(fecha, limite.tipo)
        if not filtros:
            continue
        ...
        if limite.unidad == 'LITROS':
            acumulado = ventas_periodo.aggregate(v=Sum('litros'))['v'] or 0
            intento = intento_litros
        else:
            acumulado = ventas_periodo.aggregate(v=Sum('total'))['v'] or 0
            intento = intento_monto

        if acumulado >= limite.valor:
            return _mensaje_rechazo(limite, acumulado, intento, 'ya alcanzado o excedido')
        if acumulado + intento > limite.valor:
            return _mensaje_rechazo(limite, acumulado, intento, 'excedido')
    return None
    ...
    def create(self, request):
        ...
        if data.get('cliente_id'):
            try:
                ...
                limite_error = _validar_compra_con_limites(
                    cliente=cliente,
                    fecha=timezone.localdate(),
                    intento_monto=(total or 0),
                    intento_litros=(litros or 0),
                    es_lleno=es_lleno,
                )
                if limite_error:
                    return Response({'error': limite_error}, status=status.HTTP_400_BAD_REQUEST)
