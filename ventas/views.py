...
from .models import (
    Turno, Cliente, Venta, Isla, Lado, TipoCombustible, Sucursal, Vehiculo,
    EmpresaCliente, ConfiguracionPuntos, MovimientoPuntos,
)
...
from .serializers import (
    ...
    TicketVentaSerializer, ConfiguracionPuntosSerializer, MovimientoPuntosSerializer,
)
from . import puntos_service
...
class ClienteViewSet(viewsets.ModelViewSet):
    ...
    # ── Programa de puntos ─────────────────────────────────────────────────
    @action(detail=False, methods=['get'], url_path='ranking_puntos',
            permission_classes=[IsAuthenticated])
    def ranking_puntos(self, request):
        """Lista clientes de la empresa ordenados por puntos acumulados."""
        qs = self.get_queryset().order_by('-puntos_acumulados', 'nombre')
        data = [
            {
                'id': c.id,
                'nombre': c.nombre,
    ...
    @action(detail=True, methods=['get'], url_path='puntos',
            permission_classes=[IsAuthenticated])
    def puntos(self, request, pk=None):
        """Saldo actual + historial completo de movimientos del cliente."""
    ...
    @action(detail=True, methods=['post'], url_path='ajustar_puntos',
            permission_classes=[IsAuthenticated])
    def ajustar_puntos(self, request, pk=None):
        """Ajuste manual (positivo o negativo). Body: {puntos: int, motivo: str}"""
        cliente = self.get_object()
        try:
            delta = int(request.data.get('puntos'))
        except (TypeError, ValueError):
            return Response({'error': 'puntos debe ser un entero (positivo o negativo)'},
                            status=status.HTTP_400_BAD_REQUEST)
        motivo = (request.data.get('motivo') or '').strip()
        if not motivo:
            return Response({'error': 'Debes indicar un motivo'},
                            status=status.HTTP_400_BAD_REQUEST)
...
class VentaViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        cliente = None
        puntos_a_canjear = data.get('puntos_a_canjear') or 0
        ...
        if data.get('cliente_id'):
            ...
            if puntos_a_canjear:
                if total is None:
                    return Response(
                        {'error': 'No se pueden canjear puntos en un despacho abierto ("Lleno")'},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                try:
                    descuento_puntos, _ = puntos_service.calcular_descuento_canje(
                        cliente, puntos_a_canjear, request.user.empresa,
                    )
                except ValueError as e:
                    return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
                if descuento_puntos > total:
                    return Response(
                        {'error': f'El descuento (Bs. {descuento_puntos}) supera el monto de la venta (Bs. {total})'},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                total = total - descuento_puntos
        ...
        # Aplicar canje y acumular puntos (después de crear la venta para tener el id)
        if cliente:
            if puntos_a_canjear and descuento_puntos > 0:
                puntos_service.registrar_canje(
                    cliente, puntos_a_canjear, descuento_puntos, venta, request.user,
                )
            if litros:
                puntos_service.acumular_puntos(
                    cliente, litros, request.user.empresa, venta, request.user,
                )
    ...
    def anular(self, request, pk=None):
        """Anula una venta y revierte el crédito y puntos si aplica."""
        ...
        if venta.cliente:
            puntos_service.reversar_puntos_venta(venta, request.user)
...
class ConfiguracionPuntosViewSet(viewsets.GenericViewSet):
    """Configuración del programa de puntos por empresa (una por empresa)."""
    ...
    def actual(self, request):
        """GET /configuracion-puntos/actual/ — devuelve (o crea vacía) la config de la empresa."""
        empresa = self._get_empresa(request)
        if not empresa:
            return Response({'error': 'Usuario sin empresa asignada'},
                            status=status.HTTP_400_BAD_REQUEST)
        cfg, _ = ConfiguracionPuntos.objects.get_or_create(empresa=empresa)
        return Response(ConfiguracionPuntosSerializer(cfg).data)

    @action(detail=False, methods=['put', 'patch'], url_path='guardar')
    def guardar(self, request):
        """PUT /configuracion-puntos/guardar/ — actualiza la config de la empresa."""
        empresa = self._get_empresa(request)
        if not empresa:
            return Response({'error': 'Usuario sin empresa asignada'},
                            status=status.HTTP_400_BAD_REQUEST)
        cfg, _ = ConfiguracionPuntos.objects.get_or_create(empresa=empresa)
        serializer = ConfiguracionPuntosSerializer(cfg, data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        registrar_bitacora(
            request,
            ...
            modulo='Puntos',
            descripcion=(
                f'Actualizó configuración de puntos: '
                f'{cfg.puntos_por_litro} pts/Lt, valor {cfg.valor_punto_bs} Bs, '
                f'mínimo canje {cfg.minimo_canje}, activo={cfg.activo}'
            ),
        )
        return Response(serializer.data)