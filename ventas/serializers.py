...
from .models import (
    Sucursal, Isla, Lado, TipoCombustible, Turno, Cliente, Venta, Vehiculo,
    ConfiguracionPuntos, MovimientoPuntos,
)
...
class RegistrarVentaSerializer(serializers.Serializer):
    ...
    puntos_a_canjear = serializers.IntegerField(required=False, allow_null=True, min_value=0)
...
class TicketVentaSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Venta
        fields = ['comprobante', 'sucursal', 'despacho', 'operador', 'combustible', 'pago', 'cliente', 'puntos']

    def get_puntos(self, obj):
        if not obj.cliente:
            return None
        movs = obj.movimientos_puntos.exclude(tipo='REVERSA')
        acumulados = sum(m.puntos for m in movs if m.tipo == 'ACUMULACION')
        canjeados = sum(-m.puntos for m in movs if m.tipo == 'CANJE')
        descuento_bs = 0
        for m in movs.filter(tipo='CANJE'):
            try:
                cfg = obj.turno.operador.empresa.config_puntos
                descuento_bs += float(-m.puntos) * float(cfg.valor_punto_bs)
            except Exception:
                pass
        return {
            'ganados': acumulados,
            ...
            'saldo_actual': obj.cliente.puntos_acumulados,
        }
...
class ConfiguracionPuntosSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'empresa', 'empresa_nombre', 'activo',
            'puntos_por_litro', 'valor_punto_bs', 'minimo_canje', 'updated_at',
        ]
        read_only_fields = ['empresa', 'updated_at']
...
class MovimientoPuntosSerializer(serializers.ModelSerializer):
    ...
    venta_comprobante = serializers.CharField(source='venta.numero_comprobante', read_only=True)

    class Meta:
        model = MovimientoPuntos
        fields = [
            'id', 'cliente', 'cliente_nombre', 'venta', 'venta_comprobante',
            'tipo', 'tipo_display', 'puntos', 'saldo_despues', 'descripcion',
