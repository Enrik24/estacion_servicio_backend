...
class ConfiguracionPuntos(models.Model):
    ...
    activo = models.BooleanField(default=True)
    puntos_por_litro = models.DecimalField(max_digits=6, decimal_places=2, default=1)
    valor_punto_bs = models.DecimalField(max_digits=6, decimal_places=4, default=0.10)
    ...
    def __str__(self):
        return f"Config puntos {self.empresa.nombre}"
...
class MovimientoPuntos(models.Model):
    ...
    def __str__(self):
        ...
        return f"{self.cliente.nombre} {signo}{self.puntos} pts ({self.tipo})"
