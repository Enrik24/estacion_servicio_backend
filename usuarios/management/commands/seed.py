...
from ventas.models import (
    ...
    EmpresaCliente, Turno, Venta, CompraCombustible, OrdenPrepago
)
from usuarios.models import Permiso, Rol, Usuario, Empresa, LimiteConsumo
from inventario.models import Tanque, DescargaCombustible
from monitoreo.models import EstadoSurtidor, HistorialEstadoSurtidor
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        sucursales_data = [
            {
                ...
                'tiene_gnv': False,
                'estado': 'ACTIVA',
                'latitud': -17.732977324710557,
                'longitud': -63.168478580203924,
            },
            {
                ...
                'tiene_gnv': False,
                'estado': 'ACTIVA',
                'latitud': -17.839607660578785,
                'longitud': -63.17254164678364,
            },
        ]
        ...
        for suc in sucursales_creadas.values():
            suc.tipos_combustible.set(tipos_creados.values())
        ...
        # ── Límites de Consumo ─────────────────────────────────────────────────
        self.stdout.write(self.style.NOTICE('  Creando límites de consumo...'))
        admin_user = usuarios_creados.get('admin@estacion.com')
        usuarios_para_limites = [
            usuarios_creados.get('admin@estacion.com'),
            usuarios_creados.get('gerente.norte@estacion.com'),
            usuarios_creados.get('operador@estacion.com'),
            usuarios_creados.get('auditor@estacion.com'),
        ]
        for idx, usuario in enumerate(usuarios_para_limites):
            if not usuario:
                continue
            for tipo in ['DIARIO', 'SEMANAL', 'MENSUAL']:
                valor = {0: 500, 1: 300, 2: 200, 3: 100}[idx] if tipo == 'DIARIO' else \
                        {0: 2500, 1: 1500, 2: 1000, 3: 500}[idx] if tipo == 'SEMANAL' else \
                        {0: 10000, 1: 6000, 2: 4000, 3: 2000}[idx]
                defaults = {
                    'unidad': 'MONTO',
                    'valor': valor,
                    'is_active': True,
                ...
                if tipo != 'MENSUAL':
                    defaults['fecha_inicio'] = timezone.now().date() - timedelta(days=30)
                    defaults['fecha_fin'] = timezone.now().date() + timedelta(days=30)
                LimiteConsumo.objects.get_or_create(
                    cliente=usuario,
                    tipo=tipo,
                    defaults=defaults,
                )
        self.stdout.write(self.style.SUCCESS(f'  Límites de consumo creados'))
        ...
        self.stdout.write(self.style.NOTICE('  Creando compras de combustible...'))
        operador_norte_usr = usuarios_creados.get('operador@estacion.com')
        compras_config = [
            (tipos_creados['GASOLINA_ESPECIAL'], 5000, 'Lt', 6.96, 34800),
            (tipos_creados['DIESEL'], 4000, 'Lt', 9.80, 39200),
            (tipos_creados['GASOLINA_PREMIUM'], 3000, 'Lt', 11.00, 33000),
        ]
        for tipo_obj, cantidad, unidad, precio, total in compras_config:
            CompraCombustible.objects.get_or_create(
                tipo_combustible=tipo_obj,
                cantidad=cantidad,
                unidad=unidad,
                precio_unitario=precio,
                total=total,
                created_by=operador_norte_usr,
                defaults={'observacion': 'Compra de demostración'},
            )
        self.stdout.write(self.style.SUCCESS('  Compras de combustible creadas'))
        ...
        # ── Órdenes de Prepago ─────────────────────────────────────────────────
        self.stdout.write(self.style.NOTICE('  Creando órdenes de prepago...'))
        if clientes_creados:
            clientes_prepago = clientes_creados[:3]
            for idx, cli in enumerate(clientes_prepago):
                tipo_comb = random.choice(list(tipos_creados.values()))
                monto = [100, 200, 150][idx]
                litros = round(monto / float(tipo_comb.precio_litro), 2)
                estados_prepago = ['PAGADO', 'DESPACHADO', 'PENDIENTE']
                OrdenPrepago.objects.get_or_create(
                    numero_orden=f"PRE-{idx+1:04d}",
                    defaults={
                        'cliente': cli,
                        'tipo_combustible': tipo_comb,
                        'precio_por_litro': tipo_comb.precio_litro,
                        'monto_total': monto,
                        'litros': litros,
                        'estado': estados_prepago[idx],
                        'stripe_payment_intent_id': f'pi_simulado_{idx+1}',
                        'fecha_expiracion': timezone.now() + timedelta(days=7),
                    }
                )
            self.stdout.write(self.style.SUCCESS('  Órdenes de prepago creadas'))

        # ── Historial de cambios de estado (monitoreo) ─────────────────────────
        self.stdout.write(self.style.NOTICE('  Creando historial de cambios de estado...'))
        admin_usr = usuarios_creados.get('admin@estacion.com')
        if lados_todos and admin_usr:
            for lado in lados_todos[:2]:
                for estado_anterior, estado_nuevo in [('ACTIVO', 'FALLA'), ('FALLA', 'ACTIVO')]:
                    HistorialEstadoSurtidor.objects.get_or_create(
                        lado=lado,
                        estado_anterior=estado_anterior,
                        estado_nuevo=estado_nuevo,
                        defaults={
                            'descripcion': 'Cambio de demostración',
                            'cambiado_por': admin_usr,
                        }
                    )
            self.stdout.write(self.style.SUCCESS('  Historial de cambios de estado creado'))
